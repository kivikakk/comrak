...
pub struct ComrakExtensionOptions {
    ...
    /// dbg!(options.extension.header_no_aria_hidden);
    /// assert_eq!(markdown_to_html("# README\n", &options),
    ///            "<h1><a href=\"#readme\" aria-hidden=\"true\" class=\"anchor\" id=\"user-content-readme\"></a>README</h1>\n");
    /// ```
    pub header_ids: Option<String>,

    /// Weather the headers generated by the header IDs extension should be marked as aria-hidden.
    /// If false aria-hidden is true (default), if true aria_hidden is not added to the tag.
    ///
    /// ```
    /// # use comrak::{markdown_to_html, ComrakOptions};
    /// let mut options = ComrakOptions::default();
